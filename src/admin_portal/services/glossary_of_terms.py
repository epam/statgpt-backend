...
class AdminPortalGlossaryOfTermsService(GlossaryOfTermsService):
    ...
    async def update_terms_bulk(
        ...
        if updated_item_ids:
            logger.info(
                f"Updating {len(updated_item_ids)} of {len(data)} terms: {updated_item_ids}"
            )
            await self._session.commit()
            ...
            logger.info(f"All {len(data)} terms are up-to-date.")
    ...
    async def delete_terms_bulk(
        ...
        await self._session.commit()
        logger.info(f"Deleted {len(deleted_terms)} terms: {deleted_terms}")
        return [
            schemas.GlossaryTerm.model_validate(item, from_attributes=True)
    ...
    async def export_glossary_to_folder(self, channel: models.Channel, folder_path: str) -> None:
        ...
        logger.info(f"Exporting {len(glossary_terms)} glossary terms.")
        glossary_terms_base = [
        ...
        logger.info(f"Exported glossary terms to {glossary_file!r}.")
    ...
    async def import_glossary_from_zip(self, zip_file: zipfile.ZipFile, channel_id: int) -> None:
        if JobsConfig.GLOSSARY_TERMS_FILE not in zip_file.namelist():
            logger.info("No glossary terms found in the zip file.")
            return

        logger.info("Importing glossary terms from zip file.")
        ...
        logger.info(f"Imported {len(items)} glossary terms.")
