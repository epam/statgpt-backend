...
class ExImSettings(BaseSettings):
    """
    Settings for Export-Import services
    """

    model_config = SettingsConfigDict(env_prefix="exim_")

    vector_store_concurrency_limit: int = Field(
        10, description="Maximum concurrency of export-import tasks of vector store"
    )
    elastic_concurrency_limit: int = Field(
        10, description="Maximum concurrency of export-import tasks of elasticsearch"
    )


# ToDo: either make configurable or move to constants
class JobsConfig:
