...
class PgVectorStore(VectorStore, DbServiceBase):
    ...
    async def add_documents(
        ...
        _log.info(f"Adding new documents to table '{table_name}'...")  # {len(documents)}
    ...
    async def _map_documents_with_dataset(
        ...
        for batch in batched(items, self._postgres_settings.batch_size):
            self._session.add_all(batch)
            await self._session.commit()
            _log.info(f"Added {len(batch)} document mappings for dataset(id={dataset_id})")
    ...
    async def remove_documents_by_dataset_id(self, dataset_id: int) -> None:
        ...
        _log.info(f"Deleted {len(document_ids)} documents mapping")
        if document_ids:
    ...
    async def _clear_documents_without_mapping(self, document_ids: list[int]) -> None:
        ...
        await self._session.execute(query)
        await self._session.commit()
        _log.info(f"Deleted {len(document_ids)} documents")
