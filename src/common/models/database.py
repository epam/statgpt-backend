...
class SessionMaker:
    ...
    async def _create_default_engine(self) -> AsyncEngine:
        ...
        engine = create_async_engine(postgres_settings.create_default_uri(), **self._engine_config)
    ...
    async def _create_msi_engine(self) -> AsyncEngine:
        ...
        engine = create_async_engine(postgres_settings.create_msi_uri(), **self._engine_config)
    ...
    async def create_engine(self) -> AsyncEngine:
        _log.debug(f"Creating engine (USE_MSI={postgres_settings.use_msi})")
        if postgres_settings.use_msi:
