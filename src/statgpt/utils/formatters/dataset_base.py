...
class BaseDatasetFormatter(BaseFormatter[DataSet], ABC):
    ...
    async def _get_dataset_update_at(self, dataset: DataSet) -> str:
        ...
        if last_updated_date := await dataset.updated_at(self.auth_context):
            # Use babel for proper localization of month names
            # 'MMM yyyy' format gives abbreviated month + year (e.g., "Jan 2024" or "січ 2024")
            last_updated = format_date(last_updated_date, "MMM yyyy", locale=self.config.locale)
        return last_updated
