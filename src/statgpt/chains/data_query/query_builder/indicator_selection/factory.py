...
from common.schemas import DataQueryDetails, HybridSearchConfig, IndicatorSelectionVersion
from common.utils.elastic import ElasticSearchFactory
from common.vectorstore import VectorStore
from statgpt.services.hybrid_searcher import HybridSearcher
...
class IndicatorSelectionFactory:
    ...
    async def _create_hybrid(self):
        ...
        config = self._config.hybrid_search_config or HybridSearchConfig()

        searcher = HybridSearcher(
            config, self._models_api_key, matching_index, indicators_index, self._vector_store
        )
        return IndicatorsSelectionHybrid(self._config, searcher)
