...
import statgpt.chains.data_query.query_builder.utils as query_utils
...
from statgpt.services.chat_facade import ScoredDimensionCandidate, VersionedDataSet
...
class LLMCandidateLHCL(LLMSelectionDimensionCandidate):
    ...
    @classmethod
    def candidates_to_llm_string(cls, candidates: list[LLMCandidateLHCL]) -> str:  # type: ignore[override]
        # do not include `dimension` column
        df = cls._candidates_to_df(candidates)  # type: ignore[arg-type]
...
class LHCLChainFactory(SpecialDimensionChainFactoryBase):
    ...
    async def _get_candidates_from_vector_store(
        ...
        datasets = {ds_id: ds for ds_id, ds in chain_state.datasets_dict.items()}
        version_ids = {
            datasets[ds_id].version.version_data_id for ds_id in chain_state.strong_queries.keys()
        }
