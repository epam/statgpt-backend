...
from statgpt.common.data import DataManager, DataSourceConfig, DataSourceHandler
...
class DataSourceTypeService(DbServiceBase):
    ...
    async def get_config_class(self, source_type_id: int) -> type[DataSourceConfig]:
        ...
        _log.info(f"Searching config for {item}")

        config_class = DataManager.get_config_class(item.name)
        _log.info(f"{config_class=}")
        return config_class
    ...
    async def get_data_source_handler_class_by_id(self, item_id: int) -> type[DataSourceHandler]:
        ...
        _log.info(f"Searching handler class for {item}")
