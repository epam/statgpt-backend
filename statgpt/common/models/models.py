...
from sqlalchemy import Computed, DateTime, Enum, ForeignKey, Index, String, UniqueConstraint
...
class DiscoveryDataset(DefaultBase):
    ...
    # ~~~ Descriptive fields, one per workbook column ~~~
    reference_area: Mapped[str]  # A
    regional_coverage: Mapped[str]  # B
    excluded_regional_values: Mapped[str]  # C
    agency: Mapped[str]  # D, natural key
    dataset_id: Mapped[str]  # E, natural key
    name: Mapped[str]  # F
    description: Mapped[str]  # G
    url: Mapped[str]  # H
    time_coverage: Mapped[str]  # I
    frequency_coverage: Mapped[str]  # J
    indicators_coverage: Mapped[str]  # K
    missing_indicators: Mapped[str]  # L
...
class DiscoveryIndexingJob(DefaultBase):
    ...
    __table_args__ = (Index("ix_discovery_indexing_jobs_channel_id", "channel_id"),)
