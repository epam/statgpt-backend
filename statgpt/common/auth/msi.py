...
from statgpt.common.auth.authorizer import SystemUserAuthorizeConfig, SystemUserAuthorizer
from statgpt.common.auth.base import MsiGrantI
from statgpt.common.auth.token import TokenResponseI
from statgpt.common.auth.token_cache import TokenCache
...
class CachedMsiAuthorizer(SystemUserAuthorizer):
    ...
    async def authorize(self, config: SystemUserAuthorizeConfig) -> TokenResponseI:
        ...
        if not token:
            ...
            logger.debug("MSI token has been taken from cache")
        return token
