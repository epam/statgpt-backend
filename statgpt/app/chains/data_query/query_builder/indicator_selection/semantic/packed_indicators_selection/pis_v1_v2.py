...
from statgpt.app.chains.data_query.query_builder.indicator_selection.format_utils import (
...
class IndicatorCandidatesLLMFormatter:
    ...
    def get_candidate_details_by_dataset(
        ...
        for dataset_id, dataset_candidates in cand_by_dataset.items():
            ...
            dataset_dim_values: dict[str, dict[str, str]] = {}
...
'''
    ...
    class LLMResponse(LLMResponseBase):
        ...
        async def populate_stage(self, inputs: dict) -> None:
            ...
            datasets = {k: ds.data for k, ds in chain_state.datasets_dict.items()}
    ...
    def _remove_hallucinations(self, inputs: dict) -> DatasetDimQueries:
        ...
        dataset_queries_pp: dict[str, dict[str, list[str]]] = {}  # post-processed dataset queries

        for dataset_id, query in parsed_response.get_queries().items():  # type: ignore[union-attr]
            ...
            available_dim_values = {
                dim_id: set(dim_data['values'].keys())  # type: ignore[attr-defined]
...
'''
    ...
    class LLMResponse(LLMResponseBase):
        ...
        async def populate_stage(self, inputs: dict) -> None:
            ...
            choice = ChainParameters.get_choice(inputs)
            chain_state = ChainState(**inputs)
            datasets_dict = {k: ds.data for k, ds in chain_state.datasets_dict.items()}
            ...
            if exact_is_valid or child_is_valid:
                ...
                for dataset_id in dataset_ids:
                    ...
                    formatter = SimpleDatasetFormatter(  # type: ignore[assignment]
                    ...
                    if dataset.config.citation:
                        new = await formatter.format(dataset)  # type: ignore[attr-defined]
                        lines.append(new)
