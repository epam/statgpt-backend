...
from statgpt.common.data.base import DataSetAvailabilityQuery, DimensionQuery, QueryOperator
...
class IndicatorSelectionSemanticV1ChainFactory(SemanticIndicatorSelectionBase):
    ...
    def get_indicator_selection_chain_factory(self):
        from statgpt.app.chains import (  # TODO: resolve circular import
    ...
    async def _get_indicator_candidates_from_normalized_query(
        ...
        candidates = await data_service.search_indicators_scored(
            ...
            dataset_versions=set((ds.version.version_data_id for ds in datasets_dict.values())),
        )
        return candidates
    ...
    def _filter_indicator_candidates_by_queries(
        ...
        result_list: list[ScoredIndicatorCandidate] = list(result)
        return result_list
    ...
    def _add_indicators_to_strong_availability_queries(
        ...
        for dataset_id, dataset_query in old_strong_queries.items():
            ...
            for dimension in dataset.data.indicator_dimensions(non_virtual=True):  # type: ignore[attr-defined]
        ...
        for candidate in indicator_candidates:
            ...
            for indicator in candidate.indicator.indicators:
                if query := dataset_query.dimensions_queries_dict.get(indicator.dimension_id):  # type: ignore[arg-type]
    ...
    async def _availability(inputs: dict, queries_key: str) -> dict[str, DataSetAvailabilityQuery]:
        chain_state = ChainState(**inputs)
        from statgpt.app.chains.parameters import ChainParameters  # TODO: resolve circular import
        ...
        for dataset_id, query in queries.items():
            dataset = chain_state.datasets_dict[dataset_id]
            tasks.append(dataset.data.availability_query(query, auth_context))  # type: ignore[attr-defined]
