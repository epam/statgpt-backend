...
|---|---|
| `status` | `_meta["{ns}/mcp-app"].status` |
| `message` | `_meta["{ns}/mcp-app"].message` |
...
1. Read `status` from your audience's `_meta` payload, not from `structuredContent`; the `message`
   is in `_meta["{ns}/mcp-app"]` only.
