...
| DIAL_ALLOW_DEBUG_ATTACHMENTS_TOGGLE          |    No    | Whether to advertise the `Enable debug attachments` control in the channel configuration schema, letting users switch debug attachments on themselves. Must be disabled in the production. `DIAL_SHOW_DEBUG_ATTACHMENTS` still sets the value                            | `true`, `false`                              | `false`             |
...
| MCP_RATE_LIMIT_ENABLED                       |    No    | Enforce per-caller rate limits on model-facing MCP tools. App-only tools (visibility without `model`) are always exempt.                                                                                                                             | `true`, `false`                              | `true`              |
| MCP_RATE_LIMIT_WINDOW_SECONDS                |    No    | Sustained window (seconds) over which the per-window allowances below are replenished (token-bucket refill period).                                                                                                                                  | number (seconds)                             | `60`                |
| MCP_RATE_LIMIT_CHEAP_PER_WINDOW              |    No    | Sustained per-window allowance for `cheap` tools (local lookups, e.g. glossary).                                                                                                                                                                     | integer                                      | `120`               |
| MCP_RATE_LIMIT_CHEAP_BURST                   |    No    | Burst capacity (back-to-back calls) for `cheap` tools.                                                                                                                                                                                               | integer                                      | `40`                |
| MCP_RATE_LIMIT_MODERATE_PER_WINDOW           |    No    | Sustained per-window allowance for `moderate` tools (upstream metadata reads, e.g. dataset listing/structure).                                                                                                                                       | integer                                      | `60`                |
| MCP_RATE_LIMIT_MODERATE_BURST                |    No    | Burst capacity for `moderate` tools.                                                                                                                                                                                                                 | integer                                      | `20`                |
| MCP_RATE_LIMIT_EXPENSIVE_PER_WINDOW          |    No    | Sustained per-window allowance for `expensive` tools (large SDMX queries and/or LLM-backed processing, e.g. data query).                                                                                                                             | integer                                      | `20`                |
| MCP_RATE_LIMIT_EXPENSIVE_BURST               |    No    | Burst capacity for `expensive` tools.                                                                                                                                                                                                                | integer                                      | `8`                 |
| MCP_PAYLOAD_BUDGET_ENABLED                   |    No    | Enforce a serialized-size budget on model-facing MCP tool results.                                                                                                                                                                                   | `true`, `false`                              | `true`              |
| MCP_PAYLOAD_MAX_CHARS                        |    No    | Maximum serialized size (characters) of a model-facing tool result. Kept below host caps (Claude.ai/Desktop truncate at ~150,000). Over-budget results drop heavy embedded resources first, then error asking the caller to narrow the query.        | integer (characters)                         | `120000`            |
